"""Derived textbook-fragmentation pipeline."""
...
from src.derived_sources import TEXTBOOKS_FRAGMENTED_SOURCE
...
from src.services.textbook_fragmenter import (
    TextbookFragmentationResult,
    TextbookFragmenter,
    create_textbook_fragmenter,
)
from tqdm import tqdm


DEFAULT_TEXTBOOK_FRAGMENTATION_K_VALUES = [1, 3, 5, 7, 10]
...
class TextbookFragmentedDocumentSummary:
    """Summary of one fragmented chunk document."""
...
def build_textbooks_fragmented_source_paths(workspace_paths: WorkspacePaths) -> SourcePaths:
    """Return path bundle for the derived fragmented textbook corpus."""
    source_output_dir = workspace_paths.output_root / TEXTBOOKS_FRAGMENTED_SOURCE
    return SourcePaths(
        source=TEXTBOOKS_FRAGMENTED_SOURCE,
        input_dir=workspace_paths.input_root / "textbooks",
        output_dir=source_output_dir,
        text_dir=source_output_dir / "text",
        chunks_dir=source_output_dir / "chunks",
        generation_dir=workspace_paths.output_root / "generation" / TEXTBOOKS_FRAGMENTED_SOURCE,
        retrieval_dir=workspace_paths.output_root / "retrieval" / TEXTBOOKS_FRAGMENTED_SOURCE,
        milvus_dir=workspace_paths.milvus_root / TEXTBOOKS_FRAGMENTED_SOURCE,
        results_dir=workspace_paths.results_root / TEXTBOOKS_FRAGMENTED_SOURCE,
    )


def run_textbook_fragmentation_pipeline(
    *,
    workspace_paths: WorkspacePaths,
    model_id: str | None = None,
    k_values: list[int] | None = None,
    start_at: str = "fragment",
    fragmenter: TextbookFragmenter | None = None,
    embedding_provider: EmbeddingProvider | None = None,
    vector_store: VectorStore | None = None,
    generation_llm_provider: LlmProvider | None = None,
    evaluator: SampleEvaluator | None = None,
) -> dict[str, object]:
    """Rewrite textbook chunks into study-note fragments and run the downstream pipeline."""
    if start_at not in TEXTBOOK_FRAGMENTATION_START_STAGES:
        raise ValueError(
            "start_at must be one of "
            f"{', '.join(TEXTBOOK_FRAGMENTATION_START_STAGES)}",
        )

    selected_k_values = list(k_values or DEFAULT_TEXTBOOK_FRAGMENTATION_K_VALUES)
    original_source_paths = workspace_paths.for_source("textbooks")
    fragmented_source_paths = build_textbooks_fragmented_source_paths(workspace_paths)
    original_chunk_paths = sorted(original_source_paths.chunks_dir.glob("*.json"))
    if not original_chunk_paths:
        raise FileNotFoundError(
            f"Missing textbook chunk files under {original_source_paths.chunks_dir}. "
            "Run chunk --source textbooks first.",
        )
    ...
    stage_index = TEXTBOOK_FRAGMENTATION_START_STAGES.index(start_at)
    start_stage = (
        lambda stage_name: TEXTBOOK_FRAGMENTATION_START_STAGES.index(stage_name) >= stage_index
    )
    fragmenter_model_id = model_id or ""

    document_summaries: list[TextbookFragmentedDocumentSummary] = []
    active_embedding_provider = None
    active_vector_store = None
    collection_name = chunk_collection_name(fragmented_source_paths)

    if start_at == "fragment":
        reset_textbook_fragmentation_outputs(fragmented_source_paths)
        active_fragmenter = fragmenter or create_textbook_fragmenter(model_id=model_id)
        fragmenter_model_id = active_fragmenter.model_id
        document_summaries = fragment_textbook_chunk_documents(
            input_chunk_paths=original_chunk_paths,
            output_source_paths=fragmented_source_paths,
            fragmenter=active_fragmenter,
            show_progress=True,
        )

    if start_stage("index"):
        fragmented_chunks = load_source_chunks(fragmented_source_paths)
        if start_at == "index":
            reset_textbooks_fragmented_milvus_storage(fragmented_source_paths)
        active_embedding_provider = embedding_provider or create_embedding_provider(
            show_progress=True,
        )
        active_vector_store = vector_store or create_vector_store(fragmented_source_paths.milvus_dir)
        index_chunks(
            chunks=fragmented_chunks,
            embedding_provider=active_embedding_provider,
            vector_store=active_vector_store,
            collection_name=collection_name,
        )
    elif start_stage("retrieve"):
        active_embedding_provider = embedding_provider or create_embedding_provider(
            show_progress=True,
        )
        active_vector_store = vector_store or create_vector_store(fragmented_source_paths.milvus_dir)
    ...
    for top_k in selected_k_values:
        if start_stage("retrieve"):
            ...
            retrieval_results = run_retrieval_queries(
                source_paths=fragmented_source_paths,
            ...
            retrieval_path = (
                fragmented_source_paths.retrieval_dir / f"retrieval_with_k_{top_k}.json"
            )
        ...
        if start_stage("generate"):
            ...
            generation_path = (
                fragmented_source_paths.generation_dir / f"answers_with_k_{top_k}.json"
            )
        ...
        if start_stage("evaluate"):
            ...
            per_k_summary_path = write_textbook_fragmentation_evaluation_summary(
                source_paths=fragmented_source_paths,
                top_k=top_k,
    ...
    overall_evaluation_summary_path = (
        write_textbook_fragmentation_overall_summary(
            source_paths=fragmented_source_paths,
            k_values=selected_k_values,
            per_k_summaries=evaluation_summaries,
        )
        if start_stage("evaluate")
        else fragmented_source_paths.results_dir / "SUMMARY.json"
    )
    ...
    if start_stage("ir-metrics"):
        ...
        ir_metrics_dir = ir_metrics_results_dir(fragmented_source_paths)
    ...
    fragmented_count = sum(summary.chunk_count for summary in document_summaries) - fallback_count
    summary_vector_store = active_vector_store
    if summary_vector_store is None and fragmented_source_paths.milvus_dir.exists():
        summary_vector_store = create_vector_store(fragmented_source_paths.milvus_dir)
    record_count = (
    ...
    return {
        ...
        "source": TEXTBOOKS_FRAGMENTED_SOURCE,
        "model_id": fragmenter_model_id,
        "start_at": start_at,
        "chunk_input_dir": str(original_source_paths.chunks_dir),
        "chunk_output_dir": str(fragmented_source_paths.chunks_dir),
        "rewritten_chunk_count": fragmented_count,
        ...
        "milvus_dir": str(fragmented_source_paths.milvus_dir),
        "collection_name": collection_name,
...
def fragment_textbook_chunk_documents(
    *,
    input_chunk_paths: list[Path],
    output_source_paths: SourcePaths,
    fragmenter: TextbookFragmenter,
    show_progress: bool = True,
) -> list[TextbookFragmentedDocumentSummary]:
    """Rewrite textbook chunk documents into a derived fragmented corpus."""
    summaries: list[TextbookFragmentedDocumentSummary] = []
    output_source_paths.chunks_dir.mkdir(parents=True, exist_ok=True)
    document_items: list[Path] | object = input_chunk_paths
    if show_progress:
        document_items = tqdm(
            input_chunk_paths,
            desc="Fragmenting textbooks",
            unit="document",
        )
    for input_chunk_path in document_items:
        ...
        fallback_count = 0
        fragmented_chunks: list[dict[str, object] | None] = [None] * len(chunks)
        ...
        try:
            for index, chunk in chunk_items:
                chunk_index, fragmented_chunk, used_fallback = _fragment_chunk_payload(
                    fragmenter,
                    chunk,
                    index,
                )
                fallback_count += int(used_fallback)
                fragmented_chunks[chunk_index] = fragmented_chunk
        ...
        document_metadata["source"] = TEXTBOOKS_FRAGMENTED_SOURCE
        document_metadata["derived_from_source"] = "textbooks"
        document_metadata["fragmented"] = True
            ...
            json.dumps(
                {
                    ...
                    "chunks": [chunk for chunk in fragmented_chunks if chunk is not None],
                },
                indent=2,
                ensure_ascii=True,
            )
...
def _fragment_chunk_payload(
    fragmenter: TextbookFragmenter,
    chunk: object,
    index: int,
) -> tuple[int, dict[str, object], bool]:
    """Rewrite one textbook chunk payload while preserving stable ordering metadata."""
    if not isinstance(chunk, dict):
        raise ValueError("Chunk rows must be JSON objects")

    result: TextbookFragmentationResult = fragmenter.fragment_chunk(
        chunk_text=str(chunk.get("text", "")),
        metadata=chunk,
    )
    fragmented_chunk = dict(chunk)
    fragmented_chunk["source"] = TEXTBOOKS_FRAGMENTED_SOURCE
    fragmented_chunk["text"] = result.text
    fragmented_chunk["derived_from_source"] = "textbooks"
    fragmented_chunk["fragmented"] = True
    return index, fragmented_chunk, result.used_fallback


def load_textbooks_fragmented_chunks(source_paths: SourcePaths) -> list[ChunkRecord]:
    """Load fragmented chunk records for the derived corpus."""
    return load_source_chunks(source_paths)


def reset_textbooks_fragmented_milvus_storage(source_paths: SourcePaths) -> None:
...
def reset_textbook_fragmentation_outputs(source_paths: SourcePaths) -> None:
    """Clear derived-corpus artifacts before a fresh textbook fragmentation run."""
    for directory in (
    ...
    reset_textbooks_fragmented_milvus_storage(source_paths)


def textbook_fragmentation_evaluation_summary_path(
    source_paths: SourcePaths,
    top_k: int,
) -> Path:
    """Return the per-k evaluation summary path for fragmented textbook runs."""
    return source_paths.results_dir / f"SUMMARY_with_k_{top_k}.json"


def write_textbook_fragmentation_evaluation_summary(
    source_paths: SourcePaths,
    top_k: int,
    evaluation_summary: dict[str, object],
) -> Path:
    """Write one dedicated per-k evaluation summary file."""
    summary_path = textbook_fragmentation_evaluation_summary_path(source_paths, top_k)
...
def write_textbook_fragmentation_overall_summary(
    source_paths: SourcePaths,
    k_values: list[int],
    per_k_summaries: dict[str, dict[str, object]],
) -> Path:
    """Write the shared evaluation summary as one multi-k fragmented-textbook summary."""
